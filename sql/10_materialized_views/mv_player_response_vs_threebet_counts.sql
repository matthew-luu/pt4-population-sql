...
CREATE MATERIALIZED VIEW pop.mv_player_response_vs_threebet_counts AS
SELECT
    s.id_player,
    s.position AS defend_pos,
    t.position AS threebettor_pos,
    hl.id_limit,

    COUNT(*) FILTER (
        WHERE s.flg_p_first_raise
          AND s.flg_p_3bet_def_opp
    ) AS defend_opp,

    COUNT(*) FILTER (
        WHERE s.flg_p_first_raise
          AND s.flg_p_3bet_def_opp
          AND s.enum_p_3bet_action LIKE 'C%'
    ) AS call_cnt,
...
JOIN cash_hand_player_statistics t
    ON t.id_hand = s.id_hand
